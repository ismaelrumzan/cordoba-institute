import type { QuizItem } from "@/lib/types";

export const lesson187higher: QuizItem[] = [
  {
    title:
      "True or False: The Buyids completely removed the Abbasid Caliph when they invaded Baghdad in 945.",
    type: "t/f",
    answers: [
      { title: "True", correct: false },
      { title: "False", correct: true },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "False. The Buyids did not remove the Abbasid Caliph. Instead, they reduced his power and made most of the major decisions themselves while keeping the Caliph as a figurehead.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title:
      "True or False: The period from 945 to 1055 is known as the 'Shi'a Century' because most Muslims were Shi'a during this time.",
    type: "t/f",
    answers: [
      { title: "True", correct: false },
      { title: "False", correct: true },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "False. The period is called the 'Shi'a Century' because Shi'a dynasties like the Buyids, Fatimids, and Hamdanids were in power and supported Shi'a learning. However, most Muslims remained Sunni during this period.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title:
      "True or False: Imam Ghazali is considered to be the Revivor ('mujaddid') of the 6th Islamic century.",
    type: "t/f",
    answers: [
      { title: "True", correct: true },
      { title: "False", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "True. Imam Ghazali is considered to be the Revivor ('mujaddid') of the 6th Islamic century and was given the unique title 'Hujjatul-Islam' ('the Proof of Islam') due to his significant contributions to Islamic thought.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title:
      "Which group brought about the 'Sunni revival' when they took control of Baghdad in 1055?",
    type: "single",
    answers: [
      { title: "The Buyids", correct: false },
      { title: "The Fatimids", correct: false },
      { title: "The Seljuks", correct: true },
      { title: "The Hamdanids", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "The Seljuks, a Turkic tribe, took control of Baghdad in 1055. They were Sunni, and their arrival brought about a 'Sunni revival.' They restored the power of the Abbasid Sunni Caliphate, though they maintained military and political control.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title: "What was Imam Ghazali's famous magnum opus?",
    type: "single",
    answers: [
      { title: "Kitab al-Shifa (The Book of Healing)", correct: false },
      { title: "Al-Qanun fi al-Tibb (The Canon of Medicine)", correct: false },
      {
        title: "Ihya Ulum al-Deen (Revival of the Religious Sciences)",
        correct: true,
      },
      { title: "Muqaddimah (Introduction to History)", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "Imam Ghazali's famous magnum opus was 'Ihya Ulum al-Deen' (Revival of the Religious Sciences), which he wrote after returning from his 10-year spiritual journey.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title:
      "Which university is considered to be the oldest operational university in the world?",
    type: "single",
    answers: [
      { title: "Al-Azhar University in Cairo", correct: false },
      { title: "Nizamiyya University in Baghdad", correct: false },
      { title: "Qarawiyyin University in Fez", correct: true },
      { title: "University of Bologna in Italy", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "The Qarawiyyin University in Fez, Morocco, founded by Fatima al-Fihri in 859 CE, is considered to be the oldest operational university in the world.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title:
      "Which of the following were Shi'a dynasties during the 'Shi'a Century'? (Select all that apply)",
    type: "multiple",
    answers: [
      { title: "Buyids", correct: true },
      { title: "Fatimids", correct: true },
      { title: "Hamdanids", correct: true },
      { title: "Seljuks", correct: false },
      { title: "Abbasids", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "The Buyids, Fatimids, and Hamdanids were all Shi'a dynasties during the 'Shi'a Century' (945-1055). The Seljuks were Sunni, and the Abbasids, while reduced to figureheads during this period, were also Sunni.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title:
      "Which of the following were social institutions that helped maintain cultural unity across the Islamic world during the Middle Period? (Select all that apply)",
    type: "multiple",
    answers: [
      { title: "Madhhabs (schools of law)", correct: true },
      { title: "Tariqas (Sufi orders)", correct: true },
      { title: "Madrasahs (colleges)", correct: true },
      { title: "Khaniqahs (Sufi lodges)", correct: true },
      { title: "Caliphate", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "Madhhabs (schools of law), tariqas (Sufi orders), madrasahs (colleges), and khaniqahs (Sufi lodges) were all social institutions that helped maintain cultural unity across the Islamic world during the Middle Period. The Caliphate, while historically a unifying institution, had lost much of its power and was no longer the primary source of unity during this period.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title:
      "Which of the following statements about madrasahs are true? (Select all that apply)",
    type: "multiple",
    answers: [
      {
        title:
          "They were the first institutions specifically set up to train scholars",
        correct: true,
      },
      {
        title:
          "Students from different social backgrounds came to these colleges to learn",
        correct: true,
      },
      {
        title:
          "They taught various subjects including Qur'anic studies, hadith, theology, law, and Arabic grammar",
        correct: true,
      },
      {
        title:
          "They were exclusively for religious education and did not teach secular subjects",
        correct: false,
      },
      { title: "They were controlled directly by the Caliph", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "Madrasahs were the first institutions specifically set up to train scholars, they welcomed students from different social backgrounds, and they taught various subjects including Qur'anic studies, hadith, theology, law, and Arabic grammar. They were not exclusively for religious education and were generally independent from government control.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
  {
    title:
      "Which of the following are true about Imam Ghazali? (Select all that apply)",
    type: "multiple",
    answers: [
      { title: "He was born in the town of Tus in Persia", correct: true },
      {
        title: "He was a professor at the Nizamiyya University in Baghdad",
        correct: true,
      },
      {
        title:
          "He experienced a spiritual crisis and left his position at the height of his fame",
        correct: true,
      },
      {
        title:
          "He spent 10 years on a spiritual journey before returning to teaching",
        correct: true,
      },
      {
        title: "He was a Shi'a scholar who supported the Buyid dynasty",
        correct: false,
      },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "Imam Ghazali was born in Tus, Persia, became a professor at the Nizamiyya University in Baghdad, experienced a spiritual crisis that led him to leave his position at the height of his fame, and spent 10 years on a spiritual journey before returning to teaching. He was a Sunni scholar, not a Shi'a scholar, and lived during the Seljuk period, not the Buyid dynasty.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["7higher", "the-middle-period"],
  },
];

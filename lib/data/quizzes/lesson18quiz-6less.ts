import type { QuizItem } from "@/lib/types";

export const lesson186less: QuizItem[] = [
  {
    title:
      "True or False: The Buyids completely removed the Abbasid Caliph when they took control of Baghdad.",
    type: "t/f",
    answers: [
      { title: "True", correct: false },
      { title: "False", correct: true },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "False. The Buyids did not remove the Abbasid Caliph. They let him stay as a religious leader, but they made all the important decisions themselves.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["6less", "the-middle-period"],
  },
  {
    title:
      "True or False: Imam Ghazali wrote a famous book called 'Revival of the Religious Sciences.'",
    type: "t/f",
    answers: [
      { title: "True", correct: true },
      { title: "False", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "True. After his 10-year spiritual journey, Imam Ghazali returned to teaching and wrote his famous book called 'Revival of the Religious Sciences.'",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["6less", "the-middle-period"],
  },
  {
    title:
      "Which group brought back Sunni leadership when they took control of Baghdad in 1055?",
    type: "single",
    answers: [
      { title: "The Buyids", correct: false },
      { title: "The Fatimids", correct: false },
      { title: "The Seljuks", correct: true },
      { title: "The Hamdanids", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "The Seljuks took control of Baghdad in 1055. They were Sunni Muslims, and they brought back Sunni leadership. This is called the 'Sunni revival.'",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["6less", "the-middle-period"],
  },
  {
    title:
      "Which university is the oldest university in the world that's still operating today?",
    type: "single",
    answers: [
      { title: "Al-Azhar University", correct: false },
      { title: "Nizamiyya University", correct: false },
      { title: "Qarawiyyin University", correct: true },
      { title: "Oxford University", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "The Qarawiyyin University in Fez, Morocco is considered the oldest university in the world that's still operating today. It was founded by a woman named Fatima al-Fihri in 859 CE.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["6less", "the-middle-period"],
  },
  {
    title:
      "Which of these groups were Shi'a Muslims who ruled during the 'Shi'a Century'? (Select all that apply)",
    type: "multiple",
    answers: [
      { title: "Buyids", correct: true },
      { title: "Fatimids", correct: true },
      { title: "Hamdanids", correct: true },
      { title: "Seljuks", correct: false },
      { title: "Abbasids", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "The Buyids, Fatimids, and Hamdanids were all Shi'a Muslim groups who ruled during the 'Shi'a Century' (945-1055). The Seljuks were Sunni Muslims, and the Abbasids were also Sunni.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["6less", "the-middle-period"],
  },
  {
    title: "What were madrasahs? (Select all that apply)",
    type: "multiple",
    answers: [
      { title: "Schools where scholars were trained", correct: true },
      {
        title: "Places where students from all backgrounds could learn",
        correct: true,
      },
      {
        title:
          "Schools that taught subjects like Quran, Islamic law, and Arabic",
        correct: true,
      },
      { title: "Places only for rich people", correct: false },
      { title: "Buildings only used for prayer", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "Madrasahs were schools where scholars were trained. Students from all backgrounds could learn there. They taught subjects like Quran, Islamic law, and Arabic grammar. They were not only for rich people, and they were not just buildings for prayer.",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["6less", "the-middle-period"],
  },
  {
    title:
      "Which of these facts about Imam Ghazali are true? (Select all that apply)",
    type: "multiple",
    answers: [
      { title: "He was born in Tus, Persia (now Iran)", correct: true },
      {
        title: "He was a teacher at the Nizamiyya University in Baghdad",
        correct: true,
      },
      { title: "He left his job to go on a spiritual journey", correct: true },
      {
        title: "He was a Shi'a Muslim who supported the Buyids",
        correct: false,
      },
      { title: "He never wrote any books", correct: false },
    ],
    feedback: {
      root: {
        type: "root",
        children: [
          {
            type: "paragraph",
            children: [
              {
                type: "text",
                detail: 0,
                format: 0,
                mode: "normal",
                style: "",
                text: "Imam Ghazali was born in Tus, Persia (now Iran). He was a teacher at the Nizamiyya University in Baghdad. He left his job to go on a spiritual journey for 10 years. He was a Sunni Muslim, not a Shi'a Muslim. And he wrote many books, including his famous 'Revival of the Religious Sciences.'",
                version: 1,
              },
            ],
            direction: "ltr",
            format: "",
            indent: 0,
            textFormat: 0,
            version: 1,
          },
        ],
        direction: "ltr",
        format: "",
        indent: 0,
        version: 1,
      },
    },
    tags: ["6less", "the-middle-period"],
  },
];
